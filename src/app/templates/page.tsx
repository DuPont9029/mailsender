...
export default function TemplatesPage() {
  ...
  function handleExportClick() {
    ...
    if (selectedIds.size === 0) {
      alert("Seleziona almeno un template da esportare.");
      return;
    }
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const item of json) {
        // Skip invalid items lightly
        if (!item.name || !item.subject || !item.body || !item.toEmail)
          continue;
      ...
      alert("Errore durante l'importazione: " + String(err));
    } finally {
      setImporting(false);
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
          ...
          <div className="flex gap-2">
            ...
            >
              {selectionMode ? `Conferma (${selectedIds.size})` : "Esporta"}
            </button>
            {!selectionMode && (
              <label className="px-3 py-1.5 rounded border border-gray-300 bg-white hover:bg-gray-100 text-sm cursor-pointer">
          ...
          {showCreate && (
            <div className="mt-4 border border-gray-200 rounded-lg p-4 bg-gray-50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <label className="block">
                  <span className="text-sm text-slate-700">
                    Destinatario Nome (opz.)
                  </span>
                  <input
                    className="mt-1 border border-gray-300 rounded p-2 w-full bg-white"
                    value={form.toName}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, toName: e.target.value }))
                    }
                  />
                </label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {templates.map((t, i) => {
            ...
            const isSelected = selectedIds.has(idNum);
                ...
                {selectionMode && (
                  <div className="absolute top-2 right-2 z-10">
                    <input
                      ...
                      checked={isSelected}
                      onChange={() => toggleSelection(idNum)}
                      onClick={(e) => e.stopPropagation()}
                    />
                  </div>
                )}
                <Link
                  href={selectionMode ? "#" : `/templates/${t.id}`}
                  onClick={(e) => {
                    if (selectionMode) {
                      e.preventDefault();
                      toggleSelection(idNum);
                    }
                  }}
                  className={`block border p-4 rounded-lg shadow-sm transition-colors active:scale-[0.98] h-full ${colorClass} ${
                    isSelected ? "ring-2 ring-slate-900 ring-offset-2" : ""
                  }`}
                >
                  <h2 className="font-semibold text-slate-900 pr-6">
                    {t.name}
                  </h2>
